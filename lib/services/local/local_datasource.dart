...
class LocalDataSource {
  ...
  Future<void> savePokemons(Iterable<PokemonHiveModel> pokemons) async {
    ...
    final pokemonMap = {for (var e in pokemons) e.number: e};

    await pokemonBox.clear();
    await pokemonBox.putAll(pokemonMap);
  }
  ...
  Future<List<ItemHiveModel>> getAllItems() async {
    final itemsBox = Hive.box<ItemHiveModel>(ItemHiveModel.boxKey);

    final items = List.generate(itemsBox.length, (index) => itemsBox.getAt(index)).whereType<ItemHiveModel>().toList();
